...
import IPPanels from "../components/IPPanel";
...
const SSHAttack = () => {
    ...
    const loadCommands = async () => {
      try {
        ...
        const picked = keyFound
          ? normalized[keyFound]
          : { command: "sshpass -p 'pass' ssh user@{target}" };
        ...
        const fallbackTpl = "sshpass -p 'pass' ssh user@{target}";
        setCommands({
          sshAttack: { label: "SSH Attack", command: fallbackTpl },
        });
        setCommandStr(
  ...
  useEffect(() => {
    ...
    let tpl = Array.isArray(template)
      ? template.join(" && ")
      : String(template);
    const resolved = tpl.replace(/\{target\}|\{targetIP\}/g, targetIP || "");
    setCommandStr(resolved);
  }, [targetIP, commands]);
  ...
  const handleSSHAttack = () => {
    if (!commandStr || !commandStr.trim()) {
      socket.emit("input", "# ❗ Please enter a valid command or target IP.\n");
      return;
    }

    const extracted = extractTargetFromCommand(commandStr);
    console.log("[SSHAttack] extracted target:", extracted);
    ...
    try {
      ...
      console.error("Failed to send SSH command:", err);
    } finally {
    ...
    <Layout style={{ minHeight: "100vh" }}>
      ...
      <Layout>
        ...
        <Layout>
          ...
          <IPPanels targetIP={targetIP} hostOverride="10.229.40.10:5173" />
          <TerminalPanel />
          <Content style={{ marginTop: "1px" }}>
            <div className="terminal-controls" style={{ marginBottom: "16px" }}>
              <input
                type="text"
                placeholder="Enter full command or edit"
                value={commandStr}
