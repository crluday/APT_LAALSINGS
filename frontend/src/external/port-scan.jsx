...
const API_BASE = "http://"+document.location.hostname+":3001";

const PortScan = () => {
  console.log("Port Scan component mounted");
  ...
  const extractTargetFromCommand = (cmd) => {
    ...
    if (hostname && hostname[0]) {
      if (!hostname[0].startsWith("-")) return hostname[0];
    }
  ...
  useEffect(() => {
    ...
    const loadCommands = async () => {
      try {
        ...
        const fallbackTpl = "nmap {targetIP}";
        setCommands({ portScan: { label: "Port Scan", command: fallbackTpl } });
        setCommandStr(
  ...
  const handleRunPortScan = () => {
    ...
    try {
      ...
      console.error("Failed to send command to socket:", err);
    } finally {
