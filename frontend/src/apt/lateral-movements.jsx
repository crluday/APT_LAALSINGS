...
const LateralMovementAttack = () => {
  console.log("APT: lateral-movements");
  ...
  const preferredKey = "lateral-movements";
    ...
    const loadCommands = async () => {
      try {
        ...
        const fallbackTpl =
          "sshpass -p 'anmol' ssh -tt anmol@{targetIP} 'echo crlg#123 | sudo -S bash -c";
        setCommands({
          phishing: { label: "Phishing", command: fallbackTpl },
        });
        setCommandStr(
        ...
        <Layout>
          ...
          <Content style={{ marginTop: "1px" }}>
            <div className="terminal-controls" style={{ marginBottom: "16px" }}>
              ...
              >
                {loading ? "Performing..." : "Lateral Movement Attack"}
              </button>
            </div>
          </Content>
        </Layout>
...
export default LateralMovementAttack;
