    ...
    const loadCommands = async () => {
      try {
        ...
        const picked = keyFound
          ? normalized[keyFound]
          : { command: "hydra -l admin -P passwords.txt {target}" };
        ...
        const fallbackTpl = "hydra -l admin -P passwords.txt {target}";
        setCommands({
          portScan: { label: "Brute Force", command: fallbackTpl },
        });
        setCommandStr(
        ...
        <Layout>
          ...
          <Content style={{ marginTop: "1px" }}>
            <div className="terminal-controls" style={{ marginBottom: "16px" }}>
              ...
              />
              <button
                onClick={handleBruteForceAttacks}
                disabled={loading}
                style={{ padding: "4px 12px" }}
              >
                {loading ? "Performing..." : "Brute Force Attack"}
              </button>
            </div>
          </Content>
        </Layout>
